...
static void JoinCategoriesAndProducts()
{
    using (Northwind db = new())
    {
        // объедините продукты каждой категории, чтобы в результате
        // получить 77 сопоставлений
        var queryJoin = db.Categories.Join(
        db.Products,
         category => category.CategoryId,
         product => product.CategoryId,
         (c, p) =>
        new { c.CategoryName, p.ProductName, p.ProductId }).OrderBy(ccn => ccn.CategoryName);
...
static void GroupJoinCategoriesAndProducts()
{
    using (Northwind db = new())
    {
        // сгруппируйте все товары в зависимости от их категории,
        // чтобы в результате получить восемь сопоставлений
